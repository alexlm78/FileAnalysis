...
class TestCLI:
    ...
    @patch(
        "sys.argv", ["analyze", "--file", "dummy_path.csv", "--output", "dummy_output"]
    )
    ...
    def test_cli_single_file(self, mock_to_excel, mock_analyze_file, sample_csv_path):
        ...
        with patch(
            "sys.argv",
            ["analyze", "--file", sample_csv_path, "--output", "dummy_output"],
        ):
            main()
    ...
    def test_cli_directory(
        self, mock_export_results, mock_analyze_directory, sample_directory
    ):
        ...
        with patch(
            "sys.argv",
            ["analyze", "--directory", sample_directory, "--output", "dummy_output"],
        ):
            main()
    ...
    def test_cli_no_arguments(self, mock_print_help, mock_print):
        ...
        assert any(
            "You must specify --file or --directory" in str(args[0])
